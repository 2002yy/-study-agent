...
# 1) Canonical stop catalog: exactly the values emitted by current durable writers.
path = "src/web/research/failure_contracts.py"
text = Path(path).read_text(encoding="utf-8")
text = text.replace('    "search_candidates_only",\n    "empty",\n    "candidates_only",\n    "chat_tool_loop_failed",', '    "search_candidates_only",\n    "no_tool_calls",\n    "insufficient_valid_sources",\n    "sources_read",\n    "sources_partially_read",\n    "source_reading_failed",\n    "chat_tool_loop_failed",')
if text.count('"no_tool_calls"') != 2 or '    "empty",\n' in text or '    "candidates_only",\n' in text:
    raise SystemExit("failure_contracts stop catalog replacement did not close cleanly")
Path(path).write_text(text, encoding="utf-8")

# 2) Repository is the final new-writer boundary. Readers remain intentionally open.
replace_once(
...
# 3) UI catalog mirrors backend writer truth exactly.
path = "frontend/src/features/web-lookup/researchStopReason.ts"
text = Path(path).read_text(encoding="utf-8")
text = text.replace('  "search_candidates_only",\n  "empty",\n  "candidates_only",\n  "chat_tool_loop_failed",', '  "search_candidates_only",\n  "no_tool_calls",\n  "insufficient_valid_sources",\n  "sources_read",\n  "sources_partially_read",\n  "source_reading_failed",\n  "chat_tool_loop_failed",')
text = text.replace('  search_candidates_only: "已找到候选来源，但尚未完成读取确认。",\n  empty: "本轮未获得可用的联网结果。",\n  candidates_only: "仅找到候选来源，尚未形成可确认的研究结果。",\n  chat_tool_loop_failed:', '  search_candidates_only: "已找到候选来源，但尚未完成读取确认。",\n  no_tool_calls: "本轮没有发起联网工具调用；未获得联网证据。",\n  insufficient_valid_sources: "可验证来源不足；本轮未形成足够可靠的研究结果。",\n  sources_read: "已读取并确认本轮可用来源。",\n  sources_partially_read: "部分来源已读取确认，仍有来源读取失败。",\n  source_reading_failed: "候选来源读取失败；本轮未形成可确认的读取证据。",\n  chat_tool_loop_failed:')
if text.count('"no_tool_calls"') != 1 or '  empty:' in text or '  candidates_only:' in text:
    raise SystemExit("frontend stop catalog replacement did not close cleanly")
Path(path).write_text(text, encoding="utf-8")

# 4) Learner UI prefers canonical copy and never exposes raw provider detail.
replace_once(
    "frontend/src/features/web-lookup/ChatResearchRecovery.tsx",
...
replace_once(
    "frontend/src/features/web-lookup/ChatResearchRecovery.test.tsx",
...
# 5) Replace the weak quote-scan matrix with an AST/data-flow writer audit plus runtime guard test.
matrix = r'''"""Batch-C closure matrix for failure, stop, API and UI truth.
...
'''
Path("tests/test_research_failure_state_matrix.py").write_text(matrix, encoding="utf-8")
