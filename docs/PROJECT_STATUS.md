...
- **Remote CI / current branch gate：**PR #143 final head `ab4924b1a66e15ad3a9bcdefd47af227e15f3bda` 已合并为 `main@f3f17824c132e2a88caf4dac4a9d6eae78e35910`，exact-main CI [#33947613290](https://github.com/2002yy/study-agent/actions/runs/33947613290) `push / completed / success`。Draft PR #142 最新 code-bearing exact head `a6ae861d7fb7d982a94b904d2b375a09c1975a45` 的 push CI [#34037688516](https://github.com/2002yy/study-agent/actions/runs/34037688516) 与 PR CI [#34037691314](https://github.com/2002yy/study-agent/actions/runs/34037691314) 均 `completed / success`；实际通过 full pytest **1699/1699**、RAG K1、Ruff、package helper、detect-secrets、expanded mypy baseline `122≤128 / resolved=6`、frontend tests/build、browser Golden Journeys 与 real-stack browser gates。本地聚焦集 **56/56**，本地 full pytest 为 **1697 passed / 2 Windows-only SQLite TemporaryDirectory cleanup `PermissionError`**；Linux exact-head CI 已闭合这两项。该 head 相对 `main@f3f17824` 为 `ahead 96 / behind 0`。
- **Current initiative：**Draft PR #142（分支 `codex/rq1c-bounded-qualification`）已从 infrastructure closure 进入 **RQ1-C live qualification remediation / diagnosis**。基础设施在 `c3d3576c…` 完成 fail-closed、exact-head/clean-checkout、六调用/60s pre-dispatch guard 与 5/5 review-thread closure；随后首次真实 live 12 暴露 claim planner 与 pre-read runtime 瓶颈。planner 已改为有界单次执行、question-verbatim anchor 与严格 schema，并在 exact manifest 的 planner-only diagnostic [#33983048683](https://github.com/2002yy/study-agent/actions/runs/33983048683) 取得 **12/12 completed / 0 failed**；optional supporting claim 的非 verbatim/duplicate anchor 只丢弃 optional row，critical claim 与 kind/policy 校验继续 fail closed。
- **Runtime status：**首次真实 live 12（head `cd32c209…`，run [#33979582319](https://github.com/2002yy/study-agent/actions/runs/33979582319)）只证明 artifact/protocol 执行完成，结果为 **0 reviewable answers / 0 completed runs / 10 failed / 2 partial / protocol probes 6/6**，不得视为 qualification GO。head `40624a6…` 的 pre-read diagnostic [#34029990344](https://github.com/2002yy/study-agent/actions/runs/34029990344) 将每次 assessment 限为一个物理尝试并把送审窗口降到 `max_reads`，但 1.5B assessor 仍出现 schema `TypeError`/20s timeout，两个案例继续 0 planned reads / 0 reads。head `a6ae861…` 加入 strict JSON Schema 后，pre-read diagnostic [#34037688513](https://github.com/2002yy/study-agent/actions/runs/34037688513) 的 schema 拒绝已消失，且 `rq1c-academic-primary-attention` 有一次 5-candidate assessment 完成；但该调用生成 654 tokens、耗时 18.77s，在前一波 20.02s timeout 后跨过 45s soft deadline，仍无法调度读取。`rq1c-unverifiable-python-security` 的三波 assessment 仍分别在约 20.02s / 20.02s / 15.18s timeout。两例终态依旧 **0 planned reads / 0 reads**；diagnostic workflow success 只证明安全产物生成，不能替代业务门。所有诊断 artifact 均不保存 prompt、raw response、page body 或 query text。
- **GO / NO-GO：**RQCE-P0、P1-A0、A1–A4/B1–B5 + Hardening + P1-C 子批 1/2/3 + active steering + failure-state Batch A/B/C + PR #143 answer/claim binding = **REMOTE GO / DELIVERED**。PR #142 qualification infrastructure = **GO / CLOSED at `c3d3576c…`**；当前 assessment remediation code/CI = **REMOTE GATES GREEN at `a6ae861…`**；但 pre-read business gate 与 RQ1-C 均为 **NOT GO / 0 READS / LIVE 12 FAILED**（truthfulness 12/12、quality ≥10/12、12/12 reviewable production answers均未取得），default activation = **FROZEN**；RQCE-P2 = **NOT STARTED**。
- **唯一下一步：**保持 untouched 12-case manifest、rubric、Evidence Gate、`<=20 candidates / 5–8 reads / 45s soft / 60s hard`、严格 semantic rank 与 default activation 不变，将 candidate-assessment 输入从 5–8 个进一步对齐为**首轮最多 3 个 cluster-diverse candidates**，单次 assessment timeout 上限改为 **15s**，并给两例 diagnostic 增加只含 eligibility/relevance/source-role/reason/gain 计数的安全摘要（不含 candidate id/title/URL/snippet/query/raw response）。focused 通过后走新 exact-head push/PR CI + 同两例 pre-read diagnostic；只有两例均产生真实 planned/read activity 且门禁绿，才允许重跑 untouched live 12。禁止 heuristic/legacy fallback，也不得用 workflow success、protocol 6/6 或 planner 12/12 替代最终门。
