...
- **Delivery lineage：**B5（PR #132）→ `main@5d620fe0`；B5-Hardening（PR #133）→ `main@fe308718`；P1-C 子批 1（PR #134）→ `main@c2cc60e`；P1-C 子批 2（PR #135）→ `main@533b60c`；P1-C 子批 3 + steering（PR #136）→ `main@da0c6ea`；failure-state Batch A（PR #137，head `6f0ad47`）→ `main@2c1e15c`。
- **Remote CI：**Batch A merge commit `2c1e15c` 的 exact-main CI [#33417206981](https://github.com/2002yy/study-agent/actions/runs/33417206981) 为 `push / completed / success`。**B5 + B5-Hardening + P1-C 子批 1/2/3 + active structured steering + failure-state Batch A = REMOTE GO / DELIVERED。**
- **Current initiative：**RQCE-P1 failure-state 收口；Batch B 分支 `codex/rqce-p1-failure-state-batch-b`，base `main@2c1e15c`。durable runtime failure、run stop reason、provider/read outcome 和 UI presentation 四层真值继续分离；本批只改 runtime writer/outcome，不进入 API/UI。
- **Runtime status：**active multi-wave executor、StopGate 与 structured steering 已进入 main。failure-state **Batch A 已远程交付**；**Batch B 实现提交 `7f5eb7d` 已推送**：active/recovery writer 全部 canonicalize，新 cursor production default `research-runtime-v2`，v1 dual reader 保持原 schema 不暗迁移，search/read outcome 使用 bounded local code 并把动态细节投影进 RuntimeFailure；Batch C 尚未开始。
- **GO / NO-GO：**RQCE-P0、P1-A0、A1–A4/B1–B5 + Hardening + P1-C 子批 1/2/3 + active steering + failure-state Batch A = **REMOTE GO / DELIVERED**；failure-state 合同 1A–12A = **FROZEN**；failure-state Batch B = **LOCAL PASS / PUSHED / CI PENDING / NOT DELIVERED**；RQ1 bounded/default activation = **NO-GO**；RQCE-P2 = **NOT STARTED**。
- **唯一下一步：**Batch B 创建 PR + exact-head review/CI；合并并验证 exact-main CI 后才进入 Batch C（StopGate reason typing、API canonical output、UI complete mapping + unknown fallback、writer/consumer 矩阵验收），再做 bounded 12-case qualification。P1 未通过前不进入 P2。
...
**Failure-State Batch B（LOCAL PASS / PUSHED / CI PENDING）：**分支 `codex/rqce-p1-failure-state-batch-b`，base `main@2c1e15c`，实现提交 `7f5eb7d`。新建 `ResearchRuntimeCursor` 默认写 `research-runtime-v2`；显式加载的 v1 cursor 仍按 v1 重写且 legacy raw failure 不归一化。`active_research_runtime.py` 与 model/search/read recovery writer 已统一通过 `build_runtime_failure` + deterministic failure_id + append-by-ID，一级 code 只取 frozen 8 项；policy 一次拒绝只写一条 `policy_blocked`；hard budget 只保留 stop truth，不再伪装 RuntimeFailure；generic Python type 只进 `exception_type`。search/read outcome `error_code` 分别收敛为 `search_failed` / `read_failed`，provider/read 动态事实投影到 `detail/provider_code/exception_type`。验证：聚焦 88/88 + 最终增量 7/7；全量 pytest 1510/1510（529.73s）；Ruff 全仓通过；expanded mypy `122≤128 / resolved=6`；`git diff --check` 通过。无 DB 迁移、无 StopGate/API/UI 改动、未跑真实 SearXNG/12-case qualification。尚未创建 PR，未取得最终 exact-head CI，不得标记 REMOTE GO；Batch C 仍不可开始。
