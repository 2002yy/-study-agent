...
- **Remote CI / current branch gate：**PR #143 final head `ab4924b1a66e15ad3a9bcdefd47af227e15f3bda` 已合并为 `main@f3f17824c132e2a88caf4dac4a9d6eae78e35910`，exact-main CI [#33947613290](https://github.com/2002yy/study-agent/actions/runs/33947613290) `push / completed / success`。Draft PR #142 最新 code-bearing exact head `f9c9a28a22680249f7dc8e630fdda40f910b95a8` 的 push CI [#34045278084](https://github.com/2002yy/study-agent/actions/runs/34045278084) 与 PR CI [#34045281053](https://github.com/2002yy/study-agent/actions/runs/34045281053) 均 `completed / success`；完整 CI 门包含 pytest、RAG K1、Ruff、package helper、detect-secrets、expanded mypy baseline、frontend tests/build、browser Golden Journeys 与 real-stack browser gates。本地聚焦集 **64/64**、Ruff 全仓通过、expanded mypy baseline `122≤128 / resolved=6`、`git diff --check` 通过；本地 full pytest 未因该候选重复运行，exact-head Linux CI 负责完整终审。
...
- **Runtime status：**首次真实 live 12（head `cd32c209…`，run [#33979582319](https://github.com/2002yy/study-agent/actions/runs/33979582319)）结果为 **0 reviewable answers / 0 completed runs / 10 failed / 2 partial / protocol probes 6/6**。head `bfe9343…` 的 2-candidate/300-token diagnostic [#34041825960](https://github.com/2002yy/study-agent/actions/runs/34041825960) 曾使 unverifiable case 达到 **planned=2 / read=2**，但 academic 首次有效 ranking 出现时已越过 45s soft deadline；head `2e238ab…` 的 220-token 长字段 wire 又使两例均截断。head `99f6011…` 的 `ca1` compact wire 将完整 response 降至 79–83 tokens / 4.6–7.6s，但触发 ValueError；head `f9c9a28…` 再严格兼容全零基或全一基连续序号，diagnostic [#34045278048](https://github.com/2002yy/study-agent/actions/runs/34045278048) 仍是两例各三次 `finish_reason=stop / ValueError`、**0/2 cases with reads**，response 为 **84–135 tokens / 3.1–13.1s**。时间与截断问题已消除，剩余失败属于 compact adapter 的 envelope/coverage/row/order 或领域 parser 边界；artifact 不保存 prompt、raw response、page body、query text 或 candidate identity。
- **GO / NO-GO：**RQCE-P0、P1-A0、A1–A4/B1–B5 + Hardening + P1-C 子批 1/2/3 + active steering + failure-state Batch A/B/C + PR #143 answer/claim binding = **REMOTE GO / DELIVERED**。PR #142 qualification infrastructure = **GO / CLOSED at `c3d3576c…`**；当前 assessment remediation code/CI = **REMOTE GATES GREEN at `f9c9a28…`**；pre-read 业务门仍为 **NOT GO / 0 OF 2 DIAGNOSTIC CASES READ / LIVE 12 FAILED**，default activation = **FROZEN**；RQCE-P2 = **NOT STARTED**。workflow success 不能替代业务门。
- **唯一下一步：**保持 compact wire、既有领域 parser、untouched manifest/rubric/Evidence Gate、候选池 20、总 reads 5–8、45s soft / 60s hard、严格 semantic rank、15s 单次 timeout、one-attempt-per-wave、2-candidate window 与 default activation不变；将 response JSON Schema 的 assessment 行数从静态 `1..100` 收紧为本次请求的**精确候选数**，并以不含内容/身份的异常类型区分 envelope、coverage、row、order 与 domain validation，先取得可判定诊断再修最小边界。聚焦/恢复/共享读取门 → exact-head push/PR CI → 同两例 diagnostic；只有两例均产生真实 planned/read activity 才允许 untouched live 12。禁止读取/保存 raw model response、下载/切换模型、heuristic/legacy fallback，也不得用 workflow success、protocol 6/6 或 planner 12/12 替代最终门。
