...
- **唯一下一步：**保持 untouched manifest/rubric/Evidence Gate、候选池 20、总 reads 5–8、45s soft / 60s hard、严格 semantic rank、15s 单次 timeout、one-attempt-per-wave 与 default activation 不变，将 per-claim assessment window 从 2 缩为 **1 个 cluster-diverse candidate**，对应 strict JSON 最大输出约 **200 tokens**；继续沿 durable saved-ranking 逐波推进。focused → exact-head push/PR CI → 同两例 diagnostic；只有两例均产生真实 planned/read activity 才允许 untouched live 12。禁止下载/切换模型、heuristic/legacy fallback，也不得用 workflow success、protocol 6/6 或 planner 12/12 替代最终门。
