...
- **Remote CI / current branch gate：**PR #143 final head `ab4924b1a66e15ad3a9bcdefd47af227e15f3bda` 已合并为 `main@f3f17824c132e2a88caf4dac4a9d6eae78e35910`，exact-main CI [#33947613290](https://github.com/2002yy/study-agent/actions/runs/33947613290) `push / completed / success`。Draft PR #142 最新 code-bearing exact head `71b3c259e624039e3fcd526cb3362bb58332e611` 的 push CI [#34112418741](https://github.com/2002yy/study-agent/actions/runs/34112418741) 与 PR CI [#34112423183](https://github.com/2002yy/study-agent/actions/runs/34112423183) 均 `completed / success`；完整 CI 门包含 pytest、RAG K1、Ruff、package helper、detect-secrets、expanded mypy baseline、frontend tests/build、browser Golden Journeys 与 real-stack browser gates。本地聚焦集 **64/64**（wrapper 定向 **17/17**）、Ruff 全仓通过、expanded mypy baseline `122≤128 / resolved=6`、`git diff --check` 通过；本地 full pytest 未因该候选重复运行，exact-head Linux CI 负责完整终审。
...
- **Runtime status：**首次真实 live 12（head `cd32c209…`，run [#33979582319](https://github.com/2002yy/study-agent/actions/runs/33979582319)）结果为 **0 reviewable answers / 0 completed runs / 10 failed / 2 partial / protocol probes 6/6**。head `bfe9343…` 的 2-candidate/300-token diagnostic 曾使 unverifiable case 达到 planned=2/read=2，但 academic 跨 soft deadline；后续 compact wire 把单次完整输出压到约 79–145 tokens。head `71b3c25…` 将 JSON Schema 行数锁为请求精确值并加入安全异常分类；diagnostic [#34112418750](https://github.com/2002yy/study-agent/actions/runs/34112418750) 显示 unverifiable 首波评估成功（`lead_only=2 / topic_only=2 / new_provenance_lead=2`）且产生 **read_attempt_count=2**，但页面读取均未成功，`read_count=0`；academic 首波 15s timeout、后两波均在 220 tokens 截断。两例最终仍为 **0/2 cases with successful reads**。artifact 不保存 prompt、raw response、page body、query text 或 candidate identity。
- **GO / NO-GO：**RQCE-P0、P1-A0、A1–A4/B1–B5 + Hardening + P1-C 子批 1/2/3 + active steering + failure-state Batch A/B/C + PR #143 answer/claim binding = **REMOTE GO / DELIVERED**。PR #142 qualification infrastructure = **GO / CLOSED at `c3d3576c…`**；当前 assessment remediation code/CI = **REMOTE GATES GREEN at `71b3c25…`**；pre-read 业务门仍为 **NOT GO / 0 OF 2 DIAGNOSTIC CASES SUCCESSFULLY READ / LIVE 12 FAILED**，default activation = **FROZEN**；RQCE-P2 = **NOT STARTED**。workflow success 与 read attempts 均不能替代成功读取门。
- **唯一下一步：**保持精确 2-row compact wire、既有领域 parser、untouched manifest/rubric/Evidence Gate、候选池 20、总 reads 5–8、45s soft / 60s hard、严格 semantic rank、15s 单次 timeout、one-attempt-per-wave、2-candidate window 与 default activation 不变；仅把 compact wire 的 relevance/source-role/gain-signal 长字符串改为有版本的短整数 code，server 严格映射回原 enum，以在不丢语义信息的前提下消除 academic 220-token 截断。聚焦/恢复/共享读取门 → exact-head push/PR CI → 同两例 diagnostic；若评估成功但读取仍为 0，则下一诊断只分类 reader failure，不得继续改 assessor。只有两例均产生真实成功读取才允许 untouched live 12。禁止读取/保存 raw model response、下载/切换模型、heuristic/legacy fallback，也不得用 workflow success、protocol 6/6、planner 12/12 或 read attempts 替代最终门。
