...
- **Current initiative：**RQ1-C answer/claim binding production batch（本地分支 `agent/rq1c-answer-claim-binding`，base `main@10b6316`，**共 11 commits**：`bd003ca` docs → `b594bce` binder → `76fbf9b` bounded evidence projection → `0ffa364` answer-stage audit → `686267e` publication gate → `ed9c5eb` unbound-claim publication 拒绝 → `aacb2b6` docs → P0 pre-push blockers 后新增 `6855163`（gate 仅由 server-resolved ResearchRun provenance 触发；旧/空 brief research run 仍 fail-closed；ChatRequest 无 research_sources/answer_validation 字段，无客户端覆盖路径）、`3c5a552`（answer 阶段 chat/stream 调用 `request_max_retries=0`，SDK 默认 max_retries=2 的隐藏重试不再吞掉 physical-call 计数）、`edfd2dd`（binder 外发先查 G16 `external_data_policy.web_allowed`，policy 禁止 → 0 provider calls、按普通 turn 语义处理；external_data_policy 与 answer_validation_audit 并列真值）。本地证据：focused 128 passed、full pytest 1581/1581、Ruff all clean、mypy baseline gate passed（current=122 < baseline=128）、`git diff --check` clean、worktree clean。**尚未 push/PR（无远端 CI），等待用户远端二审。**
