...
- **Remote CI / current branch gate：**PR #143 final head `ab4924b1a66e15ad3a9bcdefd47af227e15f3bda` 已合并为 `main@f3f17824c132e2a88caf4dac4a9d6eae78e35910`，exact-main CI [#33947613290](https://github.com/2002yy/study-agent/actions/runs/33947613290) `push / completed / success`。Draft PR #142 最新 code-bearing exact head `2e238ab2650654ea61916af826200183029f467c` 的 push CI [#34043354440](https://github.com/2002yy/study-agent/actions/runs/34043354440) 与 PR CI [#34043356667](https://github.com/2002yy/study-agent/actions/runs/34043356667) 均 `completed / success`；完整 CI 门包含 pytest、RAG K1、Ruff、package helper、detect-secrets、expanded mypy baseline、frontend tests/build、browser Golden Journeys 与 real-stack browser gates。本地聚焦集 **58/58**、Ruff 全仓通过、expanded mypy baseline `122≤128 / resolved=6`、`git diff --check` 通过；本地 full pytest 未因该候选重复运行，exact-head Linux CI 负责完整终审。
...
- **Runtime status：**首次真实 live 12（head `cd32c209…`，run [#33979582319](https://github.com/2002yy/study-agent/actions/runs/33979582319)）结果为 **0 reviewable answers / 0 completed runs / 10 failed / 2 partial / protocol probes 6/6**。head `40624a6…` 至 `2fddd8a…` 的 diagnostics 逐步消除了 schema 拒绝并确认 3-candidate response 在约 18 token/s runner 上跨越 soft deadline。head `bfe9343…` 的 2-candidate/300-token diagnostic [#34041825960](https://github.com/2002yy/study-agent/actions/runs/34041825960) 首次使 unverifiable case 达到 **planned=2 / read=2**，但 academic 首次有效 ranking 出现时已越过 45s soft deadline。head `2e238ab…` 将两候选总输出硬封顶 220 tokens；diagnostic [#34043354400](https://github.com/2002yy/study-agent/actions/runs/34043354400) 的两例各三次 assessment 全部在 220 tokens `finish_reason=length` 并以 `JSONDecodeError` fail closed，退化为 **0/2 cases with reads**。这证明所需输出落在现有冗长 wire 的 220–300 token 区间，继续只调 token 上限不能同时满足完整 JSON 与 soft deadline。artifact 不保存 prompt、raw response、page body、query text 或 candidate identity。
- **GO / NO-GO：**RQCE-P0、P1-A0、A1–A4/B1–B5 + Hardening + P1-C 子批 1/2/3 + active steering + failure-state Batch A/B/C + PR #143 answer/claim binding = **REMOTE GO / DELIVERED**。PR #142 qualification infrastructure = **GO / CLOSED at `c3d3576c…`**；当前 assessment remediation code/CI = **REMOTE GATES GREEN at `2e238ab…`**；pre-read 业务门仍为 **NOT GO / 0 OF 2 DIAGNOSTIC CASES READ / LIVE 12 FAILED**，default activation = **FROZEN**；RQCE-P2 = **NOT STARTED**。workflow success 不能替代业务门。
- **唯一下一步：**保持 untouched manifest/rubric/Evidence Gate、候选池 20、总 reads 5–8、45s soft / 60s hard、严格 semantic rank、15s 单次 timeout、one-attempt-per-wave、2-candidate window 与 default activation 不变；把模型边界改为**严格、定长顺序的 compact assessment wire**（短字段/候选序号），server 侧必须校验完整覆盖、顺序、enum、置信度和 gain signals，再映射回既有 `CandidateSemanticAssessment`，不得降低内部合同或接受部分结果。compact wire 聚焦/恢复/共享读取门 → exact-head push/PR CI → 同两例 diagnostic；只有两例均产生真实 planned/read activity 才允许 untouched live 12。禁止下载/切换模型、heuristic/legacy fallback，也不得用 workflow success、protocol 6/6 或 planner 12/12 替代最终门。
