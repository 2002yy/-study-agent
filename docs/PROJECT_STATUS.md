...
- **Current HEAD：**`bc650df`（`main`，与远程同步）；RQCE-P0-C5 gold-blind 修复与 live observation 当前为本地未提交工作。
- **Remote CI：**代码 checkpoint `680c5a6` 的 [#32974470552](https://github.com/2002yy/study-agent/actions/runs/32974470552) 为 `success`；最新文档 HEAD `bc650df` 的 [#32975472636](https://github.com/2002yy/study-agent/actions/runs/32975472636) 为 `failure`（52/53 browser journeys，通过项后在 360×520 complex-content 链接布局断言失败），因此当前远程 HEAD 不得标记 REMOTE GO。
...
- **GO / NO-GO：**foundation（A0–B3）REMOTE GO / DELIVERED；C1–C4 REMOTE GO（`680c5a6`）；**C5（`037e495` 已推 main，CI 未触发）+ C5-C harness closure（本地未提交）**；**20-case Shadow harness = PASS / COMPLETE**（10/10 live artifact 完整：funnel/transcript/typed_reason/stop_reason 齐备；frozen 可确定性重跑；报告可由结构化输入重生成）；**20-case diagnostic outcome = NO-GO for production activation**（eligible evidence projection 1/10；live baseline closure 是 operational search-status proxy）；RQCE-P0 production activation / RQCE-P1 / RQ1 bounded / G17-LAN 均 **NO-GO**。
- **唯一下一步：**提交 C5-C harness closure（10/10 artifact 完整 + 7-case taxonomy 已完成：RELEVANCE_FALSE_NEGATIVE=7、CLAIM_PROJECTION_UNAVAILABLE=2、COMPLETED_WITH_EVIDENCE=1），让新 SHA 跑完整远程 CI；CI 绿 + 人工复审重生成的 ``docs/research_quality/P0_SHADOW_REPORT.md``。taxonomy 显示 7/10 案例的瓶颈是 benchmark-relevance 判负（搜索返回 5 候选、0 个判相关），初步指向 legacy relevance 区分能力不足而非 query 规划；P1 目标选择须以该 taxonomy 为准。
- **RQCE-P1 前置：**不得自动进入；须先（a）C5 提交并远程 CI 绿，（b）人工确认 C5 报告，（c）P1-A 完成 8-case 失败分类、确定实现目标后，才进入 SearchIntent / Gap Planner 实际施工。
...
- **Live 10 实测：**真实公开问题已运行；搜索 API 10/10 返回 `ok`，但严格去停用词的 benchmark relevance 只有 2/10 case 有候选（50 个 URL 中 10 个相关），8/10 case 记录 provider timeout/CAPTCHA/限流；严格筛选后的正文读取 6/6 成功。`results_found` 不能解释为研究相关或证据可用。
- **Live 10 semantic projection：**已执行（8/10 completed、2/10 unavailable；14 逻辑调用/17 attempt/5 失败；projected documents 仅 4 个，全属 ``trap-simple-factual-live``；eligible evidence projection 1/10）。**dominant observed bottleneck：pre-projection retrieval coverage**——当前证据主要指向 query planning/SearchIntent 与 legacy relevance/candidate recall，但 P1 须先对 8 个 no-projection case 分类后才确定实现目标，不得直接宣布唯一根因。
