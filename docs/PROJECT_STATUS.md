...
- **Current initiative：**RQCE-P1 failure-state 枚举/映射审计；分支 `codex/rqce-p1-failure-state-audit`。本批先区分 durable runtime failure、run stop reason、provider/read outcome 和 UI presentation 四层真值，再冻结最窄兼容合同，不直接进入 P2 Reader/Synthesis/Auditor。
- **Runtime status：**active multi-wave executor、StopGate 与 structured steering 已进入 main。审计确认 `RuntimeFailure.code`、StopGate `unavailable_reason`、query/read `error_code` 与 API `stop_reason` 仍是自由字符串；UI 只覆盖部分 canonical stop reason，unknown partial 会显示原始代码。
- **GO / NO-GO：**RQCE-P0、P1-A0、A1–A4/B1–B5 + Hardening + P1-C 子批 1/2/3 + active steering = **REMOTE GO / DELIVERED**；failure-state 收口 = **AUDIT IN PROGRESS / CONTRACT NOT FROZEN**；RQ1 bounded/default activation = **NO-GO**；RQCE-P2 = **NOT STARTED**。
- **唯一下一步：**冻结 failure 与 stop 两轴合同、legacy cursor 迁移、P1/P2 原因粒度、UI unknown fallback 与 policy 去重；随后实现最窄 P1 修复并通过 deterministic gates，再一次性运行 bounded 12-case qualification。P1 未通过前不进入 P2。
