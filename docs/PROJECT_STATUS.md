...
- **Remote CI / current branch gate：**PR #143 final head `ab4924b1a66e15ad3a9bcdefd47af227e15f3bda` 已合并为 `main@f3f17824c132e2a88caf4dac4a9d6eae78e35910`，exact-main CI [#33947613290](https://github.com/2002yy/study-agent/actions/runs/33947613290) `push / completed / success`。Draft PR #142 最新 code-bearing exact head `2fddd8a14c616130d191b22eda8151c0c538da88` 的 push CI [#34039556180](https://github.com/2002yy/study-agent/actions/runs/34039556180) 与 PR CI [#34039559111](https://github.com/2002yy/study-agent/actions/runs/34039559111) 均 `completed / success`；实际通过 full pytest **1700/1700**、RAG K1、Ruff、package helper、detect-secrets、expanded mypy baseline `122≤128 / resolved=6`、frontend tests/build、browser Golden Journeys 与 real-stack browser gates。本地聚焦集 **57/57**，本地 full pytest 的 Windows-only SQLite TemporaryDirectory cleanup `PermissionError` 由 Linux exact-head CI 闭合。
...
- **Runtime status：**首次真实 live 12（head `cd32c209…`，run [#33979582319](https://github.com/2002yy/study-agent/actions/runs/33979582319)）结果为 **0 reviewable answers / 0 completed runs / 10 failed / 2 partial / protocol probes 6/6**。head `40624a6…` 的 diagnostic [#34029990344](https://github.com/2002yy/study-agent/actions/runs/34029990344) 证明单次 attempt + bounded window 仍有 schema/timeout；head `a6ae861…` 的 strict JSON Schema diagnostic [#34037688513](https://github.com/2002yy/study-agent/actions/runs/34037688513) 消除 schema 拒绝，但 5-candidate response 最长 654 tokens / 18.77s，叠加前波 timeout 后跨 soft deadline。head `df957e2…` 的 3-candidate / 15s diagnostic [#34038646002](https://github.com/2002yy/study-agent/actions/runs/34038646002) 首次产出安全 assessment 摘要：unverifiable 案例一波在 10.91s 完成 277-token response，但结果为 `lead_only=3 / semantic_unknown=3 / gain_signal=0`，调度器正确拒读；academic 三波均 15s timeout。head `2fddd8a…` 加入 pre-read lead-triage prompt 并把 3-candidate max output 收到 420 tokens；diagnostic [#34039556140](https://github.com/2002yy/study-agent/actions/runs/34039556140) 仍因本次 runner 只有约 18 token/s 而两例三波全部 15s timeout（server log 每次取消时只生成约 200–267 tokens），最终继续 **0 planned reads / 0 reads**。workflow success 只证明安全产物生成，不能替代业务门；所有 artifact 均不保存 prompt、raw response、page body、query text 或 candidate identity。
- **GO / NO-GO：**RQCE-P0、P1-A0、A1–A4/B1–B5 + Hardening + P1-C 子批 1/2/3 + active steering + failure-state Batch A/B/C + PR #143 answer/claim binding = **REMOTE GO / DELIVERED**。PR #142 qualification infrastructure = **GO / CLOSED at `c3d3576c…`**；当前 assessment remediation code/CI = **REMOTE GATES GREEN at `2fddd8a…`**；但 pre-read business gate 与 RQ1-C 均为 **NOT GO / 0 READS / LIVE 12 FAILED**，default activation = **FROZEN**；RQCE-P2 = **NOT STARTED**。
- **唯一下一步：**保持 untouched manifest/rubric/Evidence Gate、候选池 20、总 reads 5–8、45s soft / 60s hard、严格 semantic rank 与 default activation 不变，将 per-claim assessment window 从 3 缩为 **2 个 cluster-diverse candidates**，并将对应 strict JSON 最大输出从 420 收紧到约 **300 tokens**；15s 单次 timeout 与 one-attempt-per-wave 保持不变。focused → exact-head push/PR CI → 同两例 diagnostic；只有两例均产生真实 planned/read activity 才允许 untouched live 12。禁止 heuristic/legacy fallback，也不得用 workflow success、protocol 6/6 或 planner 12/12 替代最终门。
