...
- **Current initiative：**RQ1-C answer/claim binding production batch（分支 `agent/rq1c-answer-claim-binding`，base `main@10b6316`，**PR #143；14 commits**：bd003ca→2200159）。评审后修复（PR #143 comment 5530133587 的 3 P0 + 1 P1）：`391b543` **binder v2 segment 协议**——server 确定性切句（s1..sN，≤16 段/段≤1200 字符，超限 `answer_not_segmentable` fail-closed）；模型只对 server 给出的 refs 分类（kind/status + evidence_support），missing/duplicate/unknown refs、factual asserted/qualified 无正向 support、unknown evidence id、non-factual 段带 support → 整体 rejected（空 claim 集不再可能 fail-open）；claim text/确定性 id 全由 server 段文本生成（模型不发明身份）；`2200159` **G16 audit + P1**——binder outbound 在 `external_data_policy.external_calls` 记 `purpose=answer_claim_binding`（provider/data_categories/data_counts/status，无 raw error）；streaming buffering 谓词与 gate 激活统一为 `answer_validation_active`（policy web denied → 逐 token 普通流式、0 binder calls、无 audit、无 G16 binding record）；`factual_claims_fully_bound` 只认 positive support（contradicts 不满足）、qualified factual 同样必须绑定。本地证据：focused 135 passed、full pytest 1587 passed（1 个并发 timing flaky 单跑通过）、Ruff all clean、mypy baseline gate passed（122<128）、`git diff --check` clean。旧 head `d5e2ad9` 已因 semantic review 失去 merge 资格；本 head 尚未 push/无远端 CI。
