...
- **Current initiative：**RQ1-C answer/claim binding production batch（分支 `agent/rq1c-answer-claim-binding`，base `main@10b6316`，**PR #143；39 commits**，最新 production commit `b8414f0` + 1 个 handoff commit）。前序 review blockers 已用 server-owned segment/claim lineage、positive evidence semantics、Evidence Gate truth inheritance、G16 outbound audit 与 gate-aligned streaming buffering 收口。最新 unresolved P1 指出提前 disconnect/cancel 会持久化尚未通过 publication gate 的 buffered candidate；实现 `b8414f0` 统一通过 `_publishable_partial` fail closed：research validation 提前退出只可持久化/返回空 partial，普通聊天 partial 行为不变，并新增 disconnect-after-token + cancel-after-token 端到端回归。本地证据：相关 streaming/cancellation focused 13/13、full pytest 1600/1600、Ruff all clean、mypy baseline `122≤128 / resolved=6`、`git diff --check` clean。旧远端 head `3df5996` 双 CI 虽绿但已被本修复失效；最终 handoff head 尚未 push，无 exact-head CI。
