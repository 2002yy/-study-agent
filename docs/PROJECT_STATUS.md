...
- **Current initiative：**RQ1-C 下一 production batch：learner-facing final answer 的 factual claim → existing evidence_id 结构化绑定 / citation validation。复用现有 production ChatService 最终回答路径与 `ChatTurn.rag_snapshot.answer_claim_snapshot` 槽位；不在 qualification harness 内新造 answer/synthesis generator，不做 DB migration。
...
- **唯一下一步：**production final-answer claim binding batch（分支 `agent/rq1c-answer-claim-binding`，base `main@10b6316`）：补 ChatTurn 完成前结构化 factual claims → existing server-owned evidence_id 绑定 / citation validation；随后具备条件的候选 head 再运行真实 12-case qualification 并进入人工 activation Gate。P1 未通过前不进入 P2。
