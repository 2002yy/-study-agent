...
- **Runtime status：**active multi-wave executor、StopGate 与 structured steering 已进入 main。failure-state Batch B（PR #139，head `1c9e2cf`）已合并为 `main@53ac140`，但合并后自动 review 暴露 v1 recovery checkpoint 会丢 interruption marker 的真实 P2 crash/resume 回归；thread `discussion_r3896933700` unresolved。当前 hotfix 分支 `codex/rqce-p1-failure-state-v1-recovery-hotfix`，base `53ac140`；Batch C 冻结。
- **GO / NO-GO：**RQCE-P0、P1-A0、A1–A4/B1–B5 + Hardening + P1-C 子批 1/2/3 + active steering + failure-state Batch A = **REMOTE GO / DELIVERED**；failure-state 合同 1A–12A = **FROZEN**；failure-state Batch B = **REGRESSION / HOTFIX REQUIRED / NO-GO**；RQ1 bounded/default activation = **NO-GO**；RQCE-P2 = **NOT STARTED**。
- **唯一下一步：**完成 v1 recovery hotfix（v1 三字段保留 `interrupted_unknown`、v2 canonical detail 不回退、model/search/read double-crash ceiling）→ focused/full candidate-head CI → exact-head review/CI → merge → exact-main CI；全部绿后才允许进入 Batch C。P1 未通过前不进入 P2。
