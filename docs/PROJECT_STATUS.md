...
- **Remote CI / current branch gate：**PR #143 final head `ab4924b1a66e15ad3a9bcdefd47af227e15f3bda` 已合并为 `main@f3f17824c132e2a88caf4dac4a9d6eae78e35910`，exact-main CI [#33947613290](https://github.com/2002yy/study-agent/actions/runs/33947613290) `push / completed / success`。Draft PR #142 最新 code-bearing exact head `dcdacf4fd524f1bd3b54eb2c3ea15e36d8d0b7ff` 的 push CI [#34022305819](https://github.com/2002yy/study-agent/actions/runs/34022305819) 与 PR CI [#34022308276](https://github.com/2002yy/study-agent/actions/runs/34022308276) 均 `completed / success`；实际通过 full pytest **1695/1695（256.27s）**、RAG K1、Ruff、package helper、detect-secrets、expanded mypy baseline `122≤128 / resolved=6`、frontend tests/build、browser Golden Journeys 与 real-stack browser gates。该 code-bearing head 相对 `main@f3f17824` 为 `ahead 94 / behind 0`；后续 handoff 仅允许追加本文件 docs-only delta。
...
- **Runtime status：**首次真实 live 12（head `cd32c209…`，run [#33979582319](https://github.com/2002yy/study-agent/actions/runs/33979582319)）只证明 artifact/protocol 执行完成，结果为 **0 reviewable answers / 0 completed runs / 10 failed / 2 partial / protocol probes 6/6**，不得视为 qualification GO。planner 修复后的 exact-head pre-read diagnostic（head `3269a022…`，run [#34021425362](https://github.com/2002yy/study-agent/actions/runs/34021425362)）进一步锁定剩余瓶颈：`rq1c-academic-primary-attention` 收到 5 candidates、`rq1c-unverifiable-python-security` 收到 20 candidates；两者均在 planner 成功后由 candidate assessment 依次约 **20s + 20s + hard-budget remainder** 超时，最终停在 `assessing / evidence_budget_exhausted / 0 planned reads / 0 reads`。诊断 artifact 的精确禁用键检查通过，不保存 prompt、raw response、page body 或 query text。Qualification runner 仍要求 production ChatTurn answer/audit、正文 SHA-256 与真实 accounting，且不自行补造答案。
- **GO / NO-GO：**RQCE-P0、P1-A0、A1–A4/B1–B5 + Hardening + P1-C 子批 1/2/3 + active steering + failure-state Batch A/B/C + PR #143 answer/claim binding = **REMOTE GO / DELIVERED**。PR #142 qualification infrastructure = **GO / CLOSED at `c3d3576c…`**；planner contract remediation = **LOCAL/REMOTE GATES GREEN at `dcdacf4…`**；但 RQ1-C 仍为 **NOT GO / LIVE 12 FAILED**（truthfulness 12/12、quality ≥10/12、12/12 reviewable production answers 均未取得），default activation = **FROZEN**；RQCE-P2 = **NOT STARTED**。
- **唯一下一步：**在不改 untouched 12-case manifest、rubric、Evidence Gate、`<=20 candidates / 5–8 reads / 45s soft / 60s hard` 或 default activation 的前提下，窄修 **candidate-assessment pre-read budget starvation**：先用现有两案例 diagnostic/模型调用审计定义最小可验证合同，确保 assessment 不能吃完首读前的全部 hard budget，同时禁止 heuristic/legacy fallback；focused tests → 单次 full suite → exact-head push/PR CI → 重新运行同两案例 pre-read diagnostic。只有两例均产生真实 planned/read activity 且门禁绿，才允许在新 exact head 重跑 untouched live 12；任何阶段都不得把 protocol 6/6 或 planner 12/12 替代最终 truthfulness/quality 门。
