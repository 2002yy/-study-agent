...
- **Remote CI / current branch gate：**PR #143 final head `ab4924b1a66e15ad3a9bcdefd47af227e15f3bda` 已合并为 `main@f3f17824c132e2a88caf4dac4a9d6eae78e35910`，exact-main CI [#33947613290](https://github.com/2002yy/study-agent/actions/runs/33947613290) `push / completed / success`。Draft PR #142 最新 code-bearing exact head `bfe9343915553219e4046ddc470d6ea31e9a5437` 的 push CI [#34041825983](https://github.com/2002yy/study-agent/actions/runs/34041825983) 与 PR CI [#34041828977](https://github.com/2002yy/study-agent/actions/runs/34041828977) 均 `completed / success`；完整 CI 门包含 pytest、RAG K1、Ruff、package helper、detect-secrets、expanded mypy baseline、frontend tests/build、browser Golden Journeys 与 real-stack browser gates。本地聚焦集 **57/57**、Ruff 全仓通过、expanded mypy baseline `122≤128 / resolved=6`、`git diff --check` 通过；本地 full pytest 未因该候选重复运行，exact-head Linux CI 负责完整终审。
...
- **Runtime status：**首次真实 live 12（head `cd32c209…`，run [#33979582319](https://github.com/2002yy/study-agent/actions/runs/33979582319)）结果为 **0 reviewable answers / 0 completed runs / 10 failed / 2 partial / protocol probes 6/6**。head `40624a6…` 至 `2fddd8a…` 的 diagnostics 逐步消除了 schema 拒绝并确认 3-candidate response 在约 18 token/s runner 上跨越 soft deadline。head `bfe9343…` 将窗口缩为 2、上限 300 tokens，并修复后续波只评估新候选且累积旧 ranking 的恢复语义；diagnostic [#34041825960](https://github.com/2002yy/study-agent/actions/runs/34041825960) 首次使 unverifiable case 达到 **planned=2 / read=2**，但 academic case 仍为 **planned=0 / read=0**：其模型调用依次为 planner 7.98s、assessment timeout 15.02s、300-token length-stop 11.31s、有效 193-token stop 10.15s；首次可用 ranking 出现时总耗时已越过 45s soft deadline，调度器依合同不再启动读取。两例安全聚合摘要分别为 academic `lead_only=3 / rejected=1 / new_provenance_lead=2`、unverifiable `lead_only=4 / new_provenance_lead=4`；artifact 不保存 prompt、raw response、page body、query text 或 candidate identity。
- **GO / NO-GO：**RQCE-P0、P1-A0、A1–A4/B1–B5 + Hardening + P1-C 子批 1/2/3 + active steering + failure-state Batch A/B/C + PR #143 answer/claim binding = **REMOTE GO / DELIVERED**。PR #142 qualification infrastructure = **GO / CLOSED at `c3d3576c…`**；当前 assessment remediation code/CI = **REMOTE GATES GREEN at `bfe9343…`**；pre-read 业务门改善为 **1/2 diagnostic cases produced reads**，但仍是 **NOT GO / ACADEMIC 0 READS / LIVE 12 FAILED**，default activation = **FROZEN**；RQCE-P2 = **NOT STARTED**。workflow success 不能替代业务门。
- **唯一下一步：**保持 untouched manifest/rubric/Evidence Gate、候选池 20、总 reads 5–8、45s soft / 60s hard、严格 semantic rank、15s 单次 timeout、one-attempt-per-wave、**2-candidate window** 与 default activation 不变，只把该窗口 strict JSON 最大输出从 300 收紧到 **220 tokens**。曾尝试 1-candidate/200-token 本地切片，但聚焦测试立即暴露单波 stage 序列、崩溃恢复后 read 次数及多 claim 共享读取等 4 个合同回归，故未提交并按证据撤回；2-candidate/220-token 聚焦集 **58/58**。下一门为 exact-head push/PR CI → 同两例 diagnostic；只有两例均产生真实 planned/read activity 才允许 untouched live 12。禁止下载/切换模型、heuristic/legacy fallback，也不得用 workflow success、protocol 6/6 或 planner 12/12 替代最终门。
