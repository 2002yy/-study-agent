...
def match_line_range(
    *,
    ...
    for offset, source_line in enumerate(lines):
        line_tokens = set(_tokens(source_line))
        overlap = len(query_tokens & line_tokens)
        exact = int(bool(focused) and focused in source_line.casefold())
    ...
    if best is None:
        return max(1, int(chunk_start_line)), max(1, int(chunk_end_line))
    matched_line = max(1, int(chunk_start_line)) + best_offset
    return matched_line, matched_line
