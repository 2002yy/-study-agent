...
- [x] P9 web UI: React + Vite + TypeScript console implemented with SSE chat, durable refresh recovery, document upload/indexing, server-owned evidence, workflow timeline detail, controlled tool preview/call and memory status panels
- [ ] P10 hardening and integration: optional local auth/CORS implemented; Docker, production static hosting, OpenAPI examples, optional read-only MCP server, trace_id, token usage, latency and provider fallback diagnostics remain planned
